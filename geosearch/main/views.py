...
from .models import Profile, Order, Review, Category, Subcategory
...
class AuthViewSet(viewsets.ViewSet):
    ...
    def login(self, request):
        ...
        try:
            ...
            profile = Profile.objects.create(user=user, role='client', is_available=True, rating=0, completed_orders=0)
...
class SearchViewSet(viewsets.ViewSet):
    ...
    def find_performers(self, request):
        ...
        for performer in performers:
            distance = self.calculate_distance(lat, lng, performer.latitude, performer.longitude)
            if distance <= radius:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return Response({'address': address, 'display_name': address})
                except (IndexError, KeyError):
                    return Response({'error': 'Адрес не найден'}, status=404)
            return Response({'error': 'Ошибка геокодирования'}, status=500)
        except Exception as e:
...
def create_order_view(request):
    ...
    if request.method == 'POST':
        ...
        if not title or not description or not category_slug or not budget:
